...
public class TcpCanDevice : BaseDevice
{
    ...
    private bool SetIp(uint device_index)
    {
        ...
        if (ZLGApi.VCI_SetReference(UintDeviceType, device_index, canIndex, (uint)CommandType.SetDestinationIP, ref ipBytes[0]) != (uint)OperationStatus.Success)
            return false;
        return true;
    }
    ...
    private bool SetPort(uint device_index)
    {
        ...
        if (ZLGApi.VCI_SetReference(UintDeviceType, device_index, canIndex, (uint)CommandType.SetSourcePort, ref ports[0]) != (uint)OperationStatus.Success)
            return false;
        return true;
    }
}